...
const Items = ({ obj, position, firstItem, secondItems, thirdItems, secondPosition, thirdposition, lastposition }) => {
  ...
  const firstStyle = {
    ...
    position: "relative",
    margin: "0.313rem",
    transform: `translateX(${position}px) scale(.9)`,
    transition: "transform 0.3s",
  };
  const secondStyle = {
    minWidth: "9.375rem",
    position: "relative",
    margin: "0.313rem",
    transform: `translateX(${secondPosition}%) scale(.8)`,
    transition: "transform 0.3s",
  };
  const thirdStyle = {
    minWidth: "9.375rem",
    position: "relative",
    margin: "0.313rem",
    transform: `translateX(${thirdposition}%) scale(.7)`,
    transition: "transform 0.3s",
  };
  const lastStyle = {
    minWidth: "9.375rem",
    position: "relative",
    margin: "0.313rem",
    transform: `translateX(${lastposition}%) scale(.7)`,
  }
  const lastestStyle = {
    minWidth: "9.375rem",
    position: "relative",
